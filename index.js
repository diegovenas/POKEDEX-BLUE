...
const _dirname = path.resolve(path.dirname("")); //cria um avariavel e passa todas as  funcoes do modulo EXPRESS
//config

const app = express(); /*ou tbm SERVIDOR */
app.set("view engine", "ejs"); //informa o motor de visualizaçao seja em EJS.
app.use(express.static(path.join(_dirname, "public/css")));

const port = 3000; //Criando uma porta para comunicação externa! de 3000 ate 4999

app.listen(port, (req, res) => {
  //escuta o servidor o listen. Sempre seguido por 2 parametros: Um argumento e uma FUNCTION
  console.log(`ESTOU ${port}`);
});
...
const pokedexArr = pokedexObjeto.pokemon;

//criando rota
app.get("/", (req, res) => {
  res.render("index.ejs", { pokedexArr });
});

app.get('/detalhes/:id', (req, res) => {
  const pokemonAtual = pokedexArr.filter((item)=> item.id == req.params.id)
